...
parser.add_argument('--epochs', type=int, default=1270,
...
parser.add_argument('--load_dir',
                    type=str,
                    default='eval-EXP_-pso_slot46_node8_tanh_seed_3_updates_5_coeff_025_06_1_20181108-082036',
                    help='path to save the final model')
...
parser.add_argument('--evaluation', default=True, action='store_false')
parser.add_argument('--gpu', type=int, default=0, help='GPU device to use')
parser.add_argument('--arch', type=str, default='pso_start2_step0_slot_tanh_seed42', help='which architecture to use')
...
args.save = 'eval_random_{}_{}_gen_id_{}_layers_{}_seed{}_{}'.format(args.save, args.use_random,
                                   args.genotype_id,
                                             args.nlayers, args.seed,
...
# preparing the train, val and test data loaders
...
# choosing the best cell genotype to use, default DARTS in genotypes.py
if args.use_random:
    genotype, _ = swarm.get_new_genotype()
    if args.genotype_id is not None:
        genotype = swarm.genotype_from_id(args.genotype_id)
        logging.info('RANDOM CELL gen id USED, GENO: {}'.format(genotype))
    logging.info('RANDOM CELL USED, GENO: {}'.format(genotype))
else:
    genotype = eval("genotypes.%s" % args.arch)

if args.continue_train:
    logging.info('CONTINUE TRAINING, LOADING MODEL!')
    model = torch.load(os.path.join(args.load_dir, 'model.pt'))
else:
    # initializing the model given the specific genotype, discrete model
...
logging.info('Genotype: {}'.format(model.genotype()))
...
def evaluate(data_source, batch_size=10):
    ...
    for i in range(0, data_source.size(0) - 1, args.bptt):
        ...
        # TODO (ASK): preparano l'hidden state dell'ultimo step rispetto al
        # batch corrente da usare come input per il prossimo batch
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        # getting the inputs and targets of the current batch
...
try:
    if args.continue_train:
        optimizer_state = torch.load(os.path.join(args.load_dir, 'optimizer.pt'))
        logging.info('CONTINUE TRAINING, LOADING OPTIMIZER!')
        if 't0' in optimizer_state['param_groups'][0]:
            optimizer = torch.optim.ASGD(model.parameters(), lr=args.lr, t0=0, lambd=0., weight_decay=args.wdecay)
        else:
            optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, weight_decay=args.wdecay)
        optimizer.load_state_dict(optimizer_state)
        epoch = torch.load(os.path.join(args.load_dir, 'misc.pt'))['epoch']
        logging.info('EPOCH LOADED, CURRENT EPOCH IS: {}'.format(epoch))
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, weight_decay=args.wdecay)
        epoch = 1
    ...
    while epoch < args.epochs + 1:
        ...
        try:
            ...
            logging.info("exception: " + str(type(e)) + " " +  str(e.args))
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            writer.add_scalar('validation_ppl', math.exp(val_loss), epoch)
